...
;[
  ...
  test(
    "ensuring Boyer Moore Hoorspool algorithm works with static payload: " +
      JSON.stringify(payload.pattern),
    () => {
      const instance = new BoyerMooreHorspool(payload.pattern)
      const indices = instance.search(
        payload.text instanceof Buffer
          ? payload.text
...
for (let testIndex = 0, maxTests = 25; testIndex <= maxTests; testIndex++) {
  ...
  test(
    "ensuring Boyer Moore Hoorspool algorithm works with random payload: " +
      pattern,
    () => {
