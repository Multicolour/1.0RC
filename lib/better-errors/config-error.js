...
class ConfigValidationError extends PrettyErrorWithStack {
  ...
  getValidationErrorsFromAJVAST(): string[] {
    return this.validationErrors.reduce((neatErrors: string[], currentError: ErrorObject): string[] => {
      ...
      case "required":
        neatErrors.push(`Property "config${currentError.dataPath || ""}" requires the presence of "${currentError.params.missingProperty}".`)
        break
      case "minProperties":
        neatErrors.push(`Property "config${currentError.dataPath}" requires at least '${currentError.params.limit}' defined properties`)
        break
      case "enum":
      ...
      case "additionalProperties":
        neatErrors.push(`Data path config${currentError.dataPath} shouldn't have the property "${currentError.params.additionalProperty}". Maybe you misspelled the propery? Check your service's configuration.`) // @TODO: Add common typo LUT for each property to offer some answers.
        break
