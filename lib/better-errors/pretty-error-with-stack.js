...
class PrettyErrorWithStack extends Error {
  static ignored_packages: RegExp
  ...
  get_message_ast(): Error$MessageAST {
    ...
    const filtered_stack = parsed_stack
      .filter((frame_ast: Error$MessageFrameAST) => !PrettyErrorWithStack.ignored_packages.test(frame_ast.file || ""))
  ...
  to_stdout(): string {
    ...
    const messages = [
      chalk.bold.red("ERROR:", chalk.underline.italic(message_ast.message)),
      ...message_ast.stack.map((frame: Error$MessageFrameAST) => {
        return [
          "\n",
      ...
      chalk.dim("Filtered out", message_ast.frames_dropped, "frames from the stack."),
    ]
 ...
 * @type {RegExp}
 */
PrettyErrorWithStack.ignored_packages = /(module.js|bootstrap_node.js|node_modules\/flow-remove-types|next_tick.js|node_modules\/jest-jasmine2)/ // eslint-disable-line
