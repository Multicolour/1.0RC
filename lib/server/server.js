...
import type { Multicolour$IncomingMessage } from "../../flow/declarations/multicolour/incoming-message.flow"
...
class MulticolourServer {
  ...
  onResponseError(request: IncomingMessage, response: ServerResponse, error: Error) {
  ...
  async onRequest(request: Multicolour$IncomingMessage, response: ServerResponse): Promise<ServerResponse> {
    ...
    return routeMatch.handler(request, context)
      .then((reply: any) => {
        ...
        response.writeHead("content-type", context.contentType || "text/plain")
