  ...
  handleMessageFromThread(message: Multicolour$ThreadMessage): Promise<any> {
    if (!this.services.hasOwnProperty(message.serviceName))
      throw new ServiceBridgeError("Missing service dependency", [{
        ...
        message: `Received a request from a thread to query another service for data but the service it requested ("${message.serviceName}") doesn't exist. This usually happens when the target service has stopped running.`,
      }])
  }
