// @flow
// @TODO: Once happy with, PR to here
// https://github.com/flow-typed/flow-typed/pull/2421/files
...
declare type Knex$PostgresAnumOptions = { |
  useNative: boolean,
  enumName: string |
}

declare class Knex$Column {
  primary(indexName ? : string): Knex$Column;
}

declare class Knex$Table < R >
  mixins Knex$QueryBuilder < R > {
    dropColumn(column: string): Knex$Table < R > ;
    dropColumns(columns: Array < string > ): Knex$Table < R > ;
    renameColumn(column: string): Knex$Table < R > ;

    increments(name: string): Knex$Table < R > ;
    integer(name: string): Knex$Table < R > ;
    bigInteger(name: string): Knex$Table < R > ;
    text(name: string, textType ? : "text" | "mediumtext" | "longtext"): Knex$Table < R > ;
    string(name: string, length ? : number): Knex$Table < R > ;
    float(name: string, precision ? : number, scale ? : number): Knex$Table < R > ;
    decimal(name: string, precision ? : number, scale ? : number): Knex$Table < R > ;
    boolean(name: string): Knex$Table < R > ;
    date(name: string): Knex$Table < R > ;
    dateTime(name: string, precision ? : number): Knex$Table < R > ;
    time(name: string, precision ? : number): Knex$Table < R > ;
    timestamp(name: string, useTz ? : boolean, precision ? : number): Knex$Table < R > ;
    timestamps(useTimestamps ? : boolean, defaultToNow ? : boolean): Knex$Table < R > ;
    dropTimestamps(): Knex$Table < R > ;
    binary(name: string, length ? : number): Knex$Table < R > ;
    enum(name: string, values: Array < string | number > , options ? : Knex$PostgresAnumOptions): Knex$Table < R > ;
    json(name: string): Knex$Table < R > ;
    jsonb(name: string): Knex$Table < R > ;
    comment(comment: string): Knex$Table < R > ;
    engine(val: string): Knex$Table < R > ;
    collate(val: string): Knex$Table < R > ;
    inherits(val: string): Knex$Table < R > ;
    specificType(val: string, type: string): Knex$Table < R > ;
    index(columns: Array < string > , indexName ? : string, indexType ? : string): Knex$Table < R > ;
    dropIndex(columns: Array < string > , indexName ? : string): Knex$Table < R > ;
    unique(columns: Array < string > , indexName ? : string): Knex$Table < R > ;
    dropUnique(columns: Array < string > , indexName ? : string): Knex$Table < R > ;
    foreign(columns: Array < string > , indexName ? : string): Knex$Table < R > ;
    dropForeign(columns: Array < string > , indexName ? : string): Knex$Table < R > ;
    dropPrimary(constraintName ? : string): Knex$Table < R > ;
    queryContext(context: Object): Knex$Table < R > ;
    alter(): Knex$Table < R > ;
    index(indexName ? : string, indexType ? : string): Knex$Table < R > ;
  }
...
declare type Knex$RawBindings = Array < mixed > | {
  [key: string]: mixed
};
